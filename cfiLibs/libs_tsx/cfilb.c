...
cfilb_reference_monitor(unsigned long long point, unsigned long long target) {
...
__attribute__((__used__)) int cfilb_static_check(unsigned long long point,
                                                 unsigned long long target) {
  unsigned long hashIndex = (point ^ target) % HASH_KEY_RANGE;
  if (CFILB_HASH_TABLE[hashIndex] != NULL) {
    struct HashItem *temp = CFILB_HASH_TABLE[hashIndex];
...
__attribute__((__used__)) void cfilb_monitor_d0(unsigned long long point,
                                                unsigned long long target) {
  do {
    status = _xbegin();

    if (status == TSX_LOCKED) {
      unsigned long long hashIndex = ((point ^ target) % HASH_KEY_RANGE);

      if (CFILB_HASH_TABLE[hashIndex] != NULL) {
        struct HashItem *temp = CFILB_HASH_TABLE[hashIndex];
        while (temp != NULL) {
          if (temp->depth == 0 && temp->point == point &&
              temp->target == target) {
            _xend();
            return;
          }
    ...
    exit(0);
  } while ((status & _XABORT_RETRY) || (status == 0));

  fprintf(stderr, "TSX failed due to status %u\n", status);
  exit(0);
}
// reference monitor at level 1
__attribute__((__used__)) void cfilb_monitor_d1(unsigned long long point,
                                                unsigned long long target) {
  do {
    status = _xbegin();
    ...
    if (status == TSX_LOCKED) {
      ...
      unsigned long long hashIndex =
          ((site1 ^ point ^ target) % HASH_KEY_RANGE);

      if (CFILB_HASH_TABLE[hashIndex] != NULL) {
        struct HashItem *temp = CFILB_HASH_TABLE[hashIndex];
        while (temp != NULL) {
          if (temp->depth == 1 && temp->point == point &&
              temp->target == target && temp->call_site[0] == site1) {
            _xend();
            return;
          }
          temp = temp->next;
...
__attribute__((__used__)) void cfilb_monitor_d2(unsigned long long point,
                                                unsigned long long target) {
  do {
    status = _xbegin();
    ...
    if (status == TSX_LOCKED) {
      ...
      if (CFILB_HASH_TABLE[hashIndex] != NULL) {
        struct HashItem *temp = CFILB_HASH_TABLE[hashIndex];
        while (temp != NULL) {
          if (temp->depth == 2 && temp->point == point &&
              temp->target == target && temp->call_site[0] == site1 &&
...
__attribute__((__used__)) void cfilb_monitor_d3(unsigned long long point,
                                                unsigned long long target) {
  do {
    status = _xbegin();
    ...
    if (status == TSX_LOCKED) {
      ...
      unsigned long long site2 =
          (unsigned long long)__builtin_return_address(2);
      unsigned long long site3 =
          (unsigned long long)__builtin_return_address(3);
      ...
      if (CFILB_HASH_TABLE[hashIndex] != NULL) {
        struct HashItem *temp = CFILB_HASH_TABLE[hashIndex];
        while (temp != NULL) {
          if (temp->depth == 3 && temp->point == point &&
              temp->target == target && temp->call_site[0] == site1 &&
