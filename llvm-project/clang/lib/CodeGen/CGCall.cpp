  ...
  if (!(CalleePtr->stripPointerCasts())->hasName()) {
    ...
    while (1) {
      if (isa<llvm::LoadInst>(tempCalleePtr)) {
        // load instruction of callee pointer
        auto *calleeLoad = dyn_cast<llvm::LoadInst>(tempCalleePtr);

        // create an identification for the ICT
        std::string idCreator;
        llvm::raw_string_ostream rso(idCreator);
        calleeLoad->print(rso);
        idCreator +=
            ("[" + calleeLoad->getParent()->getParent()->getName().str() + "]");
        unsigned long id = hash_fn(idCreator) % 1000000000;
        ...
        llvm::Value *calleeInt = Builder.CreatePtrToInt(CalleePtr, CGM.Int64Ty);

        llvm::FunctionType *RfnType = llvm::FunctionType::get(
            CGM.VoidTy, {CGM.Int64Ty, CGM.Int64Ty}, false);
        llvm::FunctionCallee Rfn =
            CGM.CreateRuntimeFunction(RfnType, "cfilb_reference_monitor");

        llvm::CallInst *RfnCall =
            Builder.CreateCall(Rfn, {id_value_64, calleeInt});
        break;
      } else if (isa<llvm::BitCastInst>(tempCalleePtr)) {
        // a special case for explicit casting pCall
        auto *bitcast = dyn_cast<llvm::BitCastInst>(tempCalleePtr);
        tempCalleePtr = bitcast->getOperand(0);
        ...
        phi->print(rso);
        idCreator +=
            ("[" + phi->getParent()->getParent()->getName().str() + "]");
        unsigned long id = hash_fn(idCreator) % 1000000000;
        ...
        llvm::errs()
            << "[CFILB-LOG] a missing case logged ..."
            << "\n";
        llvm::errs() << "--------------------"
                     << "\n";
        tempCalleePtr->dump();
